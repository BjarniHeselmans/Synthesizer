...
void PulsatingTriangleWave(u32* inputL, u32* inputR, u32* outputL, u32* outputR, int bufferSize) {
    static float phase = 0.0f;
    static float lfo_phase = 0.0f;

    const float baseFreq = 220.0f;
    const float lfoFreq = 2.0f;
    const float phaseStep = 2.0f * M_PI * baseFreq / SAMPLE_RATE;
    const float lfoStep = 2.0f * M_PI * lfoFreq / SAMPLE_RATE;

    for (int i = 0; i < bufferSize; i++) {
        float amplitude = 0.5f * (sinf(lfo_phase) + 1.0f);
        float t = fmodf(phase / (2.0f * M_PI), 1.0f);
        float triangle = 4.0f * fabsf(t - 0.5f) - 1.0f;

        float sample = amplitude * triangle;

        // Verhoogde schaalfactor voor luider geluid
        int32_t pcmSample = (int32_t)(sample * 10000000.0f);

        outputL[i] = (u32)(pcmSample);
        outputR[i] = (u32)(pcmSample);
...
#define MAX_VOLUME 2000000.0f
#define SAMPLE_RATE 48000.0f

void RisingLaserSound(u32* outputBufferL, u32* outputBufferR, int bufferSize) {
    static float phase = 0.0f;
    static float freq = 300.0f;    // Startfrequentie
    const float maxFreq = 2000.0f;
    const float freqStep = 1.0f;   // Langzamer laten stijgen voor minder ruis

    for (int i = 0; i < bufferSize; i++) {
        float sample = sinf(phase);
        int32_t pcmSample = (int32_t)(sample * MAX_VOLUME);

        outputBufferL[i] = (u32)pcmSample;
        outputBufferR[i] = (u32)pcmSample;
...
void GeneratePianoTone(u32* outputBufferL, u32* outputBufferR, int bufferSize) {
    static float phase = 0.0f;
    const float frequency = 110.0f; // A2, lagere toon
    const float phaseStep = 2.0f * PI * frequency / SAMPLE_RATE;

    for (int i = 0; i < bufferSize; i++) {
        float t = (float)i / SAMPLE_RATE;  // tijd in seconden

        // ADSR-achtige envelope: fade-in & fade-out
        float envelope = expf(-2.0f * t);  // simpele exponentiële fading

        // Grondtoon + harmonischen
        float sample =
            1.0f   * sinf(phase) +
            0.6f   * sinf(2.0f * phase) +
            0.4f   * sinf(3.0f * phase) +
            0.3f   * sinf(4.0f * phase) +
            0.2f   * sinf(5.0f * phase) +
            0.15f  * sinf(6.0f * phase) +
            0.1f   * sinf(7.0f * phase);

        sample *= envelope;         // pas amplitude aan over tijd
        sample *= 0.4f;             // volume naar veilige waarde

        int32_t pcmSample = (int32_t)(sample * 3000000.0f);
        outputBufferL[i] = (u32)pcmSample;
        outputBufferR[i] = (u32)pcmSample;
...
static void Timer_ISR(void *CallBackRef)
{
    ...
    // Read GPIO inputs
    u32 buttonStatus = XGpio_DiscreteRead(&gpio_0, 1); // Kanaal 1 voor buttons
    u32 switchStatus = XGpio_DiscreteRead(&gpio_0, 2); // Kanaal 2 voor switches
    ...
    switch (buttonStatus)
    {
        ...
        default:
        	switch (switchStatus)
        	    {
        			case 1:
        					xil_printf("switch 1! PulsatingTriangleWave\n\r");
        					PulsatingTriangleWave(&inputL, &inputR, &outputL, &outputR, 1);
        					break;
        			case 2:
        					xil_printf("switch 2! RisingLaserSound\n\r");
        					RisingLaserSound(&outputL, &outputR, 1);
        					break;
        	        default:
        	            outputL = inputL;
        	            outputR = inputR;
